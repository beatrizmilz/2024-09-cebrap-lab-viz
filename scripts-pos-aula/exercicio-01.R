...
mananciais |>
  ...
  mutate(
    ...
    # extraindo o dia
    dia = day(data),
    # arredondando 'para baixo' a data
    mes_arredondado = floor_date(data, "month"),
    .after = data
  )
...
mananciais |> 
  ...
  ggplot() + 
  aes(x = sistema, y = volume_porcentagem) +
  geom_col(aes(fill = sistema), show.legend = FALSE)
...
mananciais |>
  ...
  # fct_reorder(coluna_de_texto_para_reordenar_como_categorias, coluna_numerica)
  mutate(sistema_fator = fct_reorder(sistema, -volume_operacional)) |> 
   # arrange(desc(volume_operacional)) |> 
  ggplot() +
  aes(x = sistema_fator, y = volume_operacional) +
  geom_col(aes(fill = sistema_fator), show.legend = FALSE)


graf_col_absoluto_ordemcre <- mananciais |>
  filter(data == max(data)) |>
  mutate(sistema_fator = fct_reorder(sistema, -volume_operacional)) |> 
  ggplot() +
  aes(x = sistema_fator, y = volume_operacional) +
...
# ------ dúvida da marina: como colocar a curva da distribuição normal
...
pinguins |> 
  ...
  aes(massa_corporal) +
  geom_density() +
  stat_function(fun = ~dnorm(x = .x,
                             mean = media,
                             sd = desvio_padrao), color = "red")
