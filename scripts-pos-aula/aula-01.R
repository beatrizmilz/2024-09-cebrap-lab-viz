...
dados_pnud_2010 |> 
  # aes dentro do ggplot
  ggplot(aes(x = idhm, y = espvida)) +
  geom_point()  +
  geom_smooth(method = "lm", se = FALSE) 
...
dados_pnud_2010 |> 
  ggplot() +
  # aes dentro da geometria
  geom_point(aes(x = idhm, y = espvida)) +
  geom_smooth(method = "lm", se = FALSE) 
...
# precisa indicar a base de dados

ggplot(aes(x = idhm, y = espvida), data = dados_pnud_2010) +
  geom_point()
...
media_idhm_por_regiao <- dados_pnud_2010 |>
  ...
  summarise(media_idhm = mean(idhm), .groups = "drop") # |> 
  #ungroup()

# `summarise()` has grouped
# output by 'regiao_nm'. You can
# override using the `.groups`
# argument.

# Deixar os parentesis coloridos
# Tools > global options > code > display > use rainbow parentesis
media_idhm_por_regiao <- dados_pnud_2010 |>
  group_by(regiao_nm, uf_sigla) |>
...
# gráfico de contagem de frequencias,
# podemos fazer com geom_bar ou geom_col
dados_pnud_2010 |> 
  ggplot() +
  geom_bar(aes(x = regiao_nm))

dados_pnud_2010 |> 
  count(regiao_nm, sort = TRUE) |> 
  ggplot() +
  geom_col(aes(x = regiao_nm, y = n))
...
dados_pnud_2010 |> 
  ...
  aes(x = idhm, y = espvida) +
  geom_point() +
  gghighlight(espvida > 78.5, label_key = muni_nm)

dados_pnud_2010 |> 
  ggplot() +
  aes(x = idhm, y = espvida) +
  geom_point() +
  gghighlight(muni_nm %in% c("DIADEMA", "OSASCO"), label_key = muni_nm)

# eu consigo por exemplo escolher o tipo de linha e o tipo de categoria?
# exemplo: pontilhar apenas a linha dos municípios do sul?

dados_sul <- dados_idhm_ano_regiao |> 
  filter(regiao_nm == "Sul")

dados_outros <- dados_idhm_ano_regiao |> 
  filter(regiao_nm != "Sul")
...
dados_outros |>
  ...
  aes(x = ano, y = media_idhm) +
  geom_line(aes(color = regiao_nm)) +
  geom_line(aes(color = regiao_nm), linetype = "dashed", data = dados_sul)
...
dados_pnud_2010 |>
  ggplot() +
  aes(x = idhm) +
  geom_histogram(bins = 10, fill = "#ddf4ff")
...
# Tem como colocar uma curva normal desenhada,
# como tinha a de tendência?
...
dados_pnud_2010 %>%
 filter(regiao_nm %in% "Sudeste") %>%
 ggplot() +
 aes(x = idhm, y = espvida, colour = uf_sigla) +
 geom_point(size = 1.15, shape = "square") +
 geom_smooth(aes(x = idhm, y = espvida), se = TRUE) +
 scale_color_viridis_d(option = "inferno", direction = 1) +
 labs(title = "exemplo de gráfico") +
 theme_light() +
 theme(axis.text.x = element_text(face = "bold", 
 size = 15L), legend.text = element_text(face = "bold"))
distribuição dos valores de esperança de vida por região
dados_pnud_2010 |>
  ggplot() +
...
dados_pnud_2010 |>
  ggplot() +
  aes(x = regiao_nm, y = espvida) +
  geom_boxplot(aes(fill = regiao_nm), color = "black") +
  geom_jitter(alpha = 0.3)


# tentando gráfico de pizza
# https://r-graph-gallery.com/piechart-ggplot2.html
dados_pnud_2010 |> 
  ggplot() +
  aes(x = "") +
  geom_bar(aes(fill = regiao_nm))  +
  coord_polar("y", start = 0) 


# Experimentando o esquisse
library(esquisse)
# esquisse:::esquisser()
dados_pnud_2010 %>%
 filter(regiao_nm %in% "Sudeste") %>%
